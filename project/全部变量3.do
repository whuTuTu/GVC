...
sum PatentApply PatentGet Patent
...
keep Symbol Year Province City 
save .\input\上市公司地区和行业信息.dta,replace
// 省级层面统计专利数据(被GVC投资过企业)
use .\input\企业专利数据.dta,clear
...
// merge 1:1 Symbol Year using .\input\控制变量2_企业ST信息.dta,nogen
// drop if ST == "Y"
...
replace VC_num=0 if VC_num==.
drop if Year<2000
gen Ind = substr(Nindcd,1,1)
drop if Ind == "J"

// winsor
local vv "ROA Growth Cash MB Tang Boardsize Balance InsInvest ManShare IndDirector Age Z SOE LMV LEV RD Patent PatentGet PatentApply"
